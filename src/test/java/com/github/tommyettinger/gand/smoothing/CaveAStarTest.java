...
public class CaveAStarTest extends PathFinderTestBase {
	...
	boolean[][] worldMap = new boolean[125][75];
	{
		for (int x = 0; x < 125; x++) {
			for (int y = 0; y < 75; y++) {
				worldMap[x][y] = caves[x].charAt(y) == '.';
			}
		}
	}
	...
	public void create () {
		...
		updatePath(true);
		pathSmoother = new PathSmoother<>(new Ortho2DRaycastCollisionDetector<PointF2>((x, y) -> x >= 0 && x < worldMap.length && y >= 0 && y < worldMap[x].length && worldMap[x][y]));
...
//		pathSmoother = new PathSmoother<>(new CharBresenhamRaycastCollisionDetector(worldMap));
	...
	private void updatePath (boolean forceUpdate) {
		...
		if (forceUpdate || tileX != lastEndTileX || tileY != lastEndTileY) {
			if(pathFinder == null) {
				pathFinder = new Float2UndirectedGraph(worldMap, 1f, PointF2::dst, diagonal);
				storedDiagonal = diagonal;
			}
			else if(storedDiagonal != diagonal) {
