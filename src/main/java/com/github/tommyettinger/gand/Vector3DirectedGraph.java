...
public class Vector3DirectedGraph extends DirectedGraph<Vector3> implements Json.Serializable {
    public Vector3DirectedGraph() {
        super();
    }

    public Vector3DirectedGraph(Collection<Vector3> vertices) {
        super(vertices);
    }

    public Vector3DirectedGraph(Collection<Vector3> vertices, Collection<Edge<Vector3>> edges, float defaultEdgeWeight) {
        super(vertices, edges, defaultEdgeWeight);
    }

    public Vector3DirectedGraph(Graph<Vector3> graph) {
        super(graph);
    }
     ...
     */
    public Vector3DirectedGraph(boolean[][][] validGrid, float defaultEdgeWeight){
        super();
     ...
     * @param defaultEdgeWeight the default edge weight to use when a weight is unspecified
     */
    public Vector3DirectedGraph(char[][][] validGrid, char validChar, float defaultEdgeWeight){
        super();
        setDefaultEdgeWeight(defaultEdgeWeight);
        for (int z = 0; z < validGrid.length; z++) {
            for (int y = 0; y < validGrid[z].length; y++) {
     ...
     * @param defaultEdgeWeight the default edge weight to use when a weight is unspecified
     */
    public Vector3DirectedGraph(float[][][] validGrid, float minimumThreshold, float maximumThreshold, float defaultEdgeWeight){
        super();
        setDefaultEdgeWeight(defaultEdgeWeight);
        for (int z = 0; z < validGrid.length; z++) {
            for (int y = 0; y < validGrid[z].length; y++) {
    ...
    @Override
    public Vector3DirectedGraph createNew() {
        return new Vector3DirectedGraph();
    }
...
//        // Harmonious numbers
    ...
    public String toString() {
        return "Vector3DirectedGraph: { size=" + size() + " }";
    }
